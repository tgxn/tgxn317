...
public class RSInterface {
    ...
    public static void unpack(JagexArchive interfaceIndex, TextDrawingArea fonts[], JagexArchive spriteArchive) {
        ...
        Packet data = new Packet(interfaceIndex.getDataForName("data"));
        int child = -1;
    ...
    private Model getModelByType(int modelType, int modelID) {
        Model model = (Model) modelNodes.insertFromCache((modelType << 16) + modelID);
        if (model != null) {
            return model;
        }
        if (modelType == 1) {
        ...
        if (modelType == 2) {
            model = NPCDef.forID(modelID).getEntityModel();
        }
        if (modelType == 3) {
        ...
        if (model != null) {
            modelNodes.removeFromCache(model, (modelType << 16) + modelID);
        }
        return model;
    }
    ...
    private static Sprite getSprite(int i, JagexArchive spriteArchive, String s) {
        ...
        Sprite cacheSprite = (Sprite) spriteNodes.insertFromCache(l);
        if (cacheSprite != null) {
            return cacheSprite;
        }
        try {
            cacheSprite = new Sprite(spriteArchive, s, i);
            spriteNodes.removeFromCache(cacheSprite, l);
    ...
    public static void getRegularModel(boolean flag, Model model) {
        ...
        if (model != null) {
            modelNodes.removeFromCache(model, (5 << 16));
        }
    }
    ...
    public Model getAnimatedModel(int id, int k, boolean enabled) {
        ...
        if (k == -1 && id == -1 && animatedModel.trianglecolours == null) {
            return animatedModel;
        }
        Model model_1 = new Model(9, true, AnimationHeader.method532(k) & AnimationHeader.method532(id), false, animatedModel);
        if (k != -1 || id != -1) {
