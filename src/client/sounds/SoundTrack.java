...
public class SoundTrack {
    ...
    private AmplitudeEnvelope samp7;
    private AmplitudeEnvelope samp8;
    ...
    public final int[] buildSoundData(int sampleLength, int j) {
        ...
        for (int k2 = 0; k2 < sampleLength; k2++) {
            ...
            if (samp3 != null) {
                ...
                int j6 = samp4.currentAmplitude(sampleLength);
                l2 += getValue(j6, j1, samp3.form) >> 1;
                j1 += (j5 * l >> 16) + i1;
            }
            if (samp5 != null) {
                int k5 = samp5.currentAmplitude(sampleLength);
                int k6 = samp6.currentAmplitude(sampleLength);
                j4 = j4 * ((getValue(k6, i2, samp5.form) >> 1) + 32768) >> 15;
                i2 += (k5 * k1 >> 16) + l1;
            }
            for (int l5 = 0; l5 < 5; l5++) {
                if (anIntArray106[l5] != 0) {
                    ...
                    if (l6 < sampleLength) {
                        sampleBuffer[l6] += getValue(j4 * anIntArray120[l5] >> 15, phase[l5], samp1.form);
        ...
        if (samp7 != null) {
            ...
            for (int i7 = 0; i7 < sampleLength; i7++) {
                int k7 = samp7.currentAmplitude(sampleLength);
                int i8 = samp8.currentAmplitude(sampleLength);
                int k4;
                if (flag1) {
        ...
        if (aClass39_111.anIntArray665[0] > 0 || aClass39_111.anIntArray665[1] > 0) {
            amplitudeEnvelope.resetValues();
            int k3 = amplitudeEnvelope.currentAmplitude(sampleLength + 1);
            ...
            if (sampleLength >= i5 + i6) {
                ...
                for (; j7 < l7; j7++) {
                    ...
                    sampleBuffer[j7] = j8;
                    k3 = amplitudeEnvelope.currentAmplitude(sampleLength + 1);
                }
                char c = '\200';
                l7 = c;
                do {
                ...
                for (; j7 < sampleLength; j7++) {
                    ...
                    sampleBuffer[j7] = i9;
                    int l3 = amplitudeEnvelope.currentAmplitude(sampleLength + 1);
                }
            }
        }
    ...
    public final void unpack(Stream stream) {
        ...
        if (i != 0) {
            stream.currentOffset--;
            samp7 = new AmplitudeEnvelope();
            samp7.method325(stream);
            samp8 = new AmplitudeEnvelope();
            samp8.method325(stream);
        }
        for (int j = 0; j < 10; j++) {
        ...
        amplitudeEnvelope = new AmplitudeEnvelope();
        aClass39_111.method545(stream, amplitudeEnvelope);
    }
}
